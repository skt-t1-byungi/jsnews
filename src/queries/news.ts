...
export async function writeNewsQuery(arg: { authorId: number; title: string; contents: string }) {
    const isTooMany = await db
        ...
        .from(news)
        .where(q.and(q.eq(news.authorId, arg.authorId), q.gte(news.createdAt, dayjs().subtract(30, 'second').toDate())))
...
export function editNewsQuery(arg: { id: number; title: string; contents: string }) {
    return db.update(news).set({ title: arg.title, contents: arg.contents }).where(q.eq(news.id, arg.id))
}
